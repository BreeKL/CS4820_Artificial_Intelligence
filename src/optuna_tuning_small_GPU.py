...
class OptunaTrainerGPU:
    ...
    def __init__(
        ...
        train_data: Dict,
        val_data: Dict,
        n_trials: int = 30, # Fewer trials to run faster
    ...
    def create_model_config(self, trial: Trial) -> Dict:
        """Create model configuration from Optuna trial - SMALL DATASET VERSION."""
        # Smaller models for small dataset
        d_model = trial.suggest_categorical('d_model', [64, 128, 256])  # Changed
        n_heads = trial.suggest_categorical('n_heads', [4, 8])
        n_layers = trial.suggest_int('n_layers', 2, 4)  # Changed from 6
        
        while d_model % n_heads != 0:
            n_heads = 4 if n_heads == 8 else 8
        
        d_ff = trial.suggest_categorical('d_ff', [256, 512, 1024])  # Changed from 2048
    ...
    def create_training_config(self, trial: Trial) -> Dict:
        """Create training configuration - SMALL DATASET VERSION."""
        learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True)
        weight_decay = trial.suggest_float('weight_decay', 1e-5, 1e-2, log=True)
        
        # Smaller batches for small dataset
        batch_size = trial.suggest_categorical('batch_size', [8, 16])  # Changed
        ...
        training_config = {
            ...
            'num_epochs': 10,  # Changed from 20
            'learning_rate': learning_rate,
            'weight_decay': weight_decay,
            'optimizer': 'adamw',
            'scheduler_patience': 3,  # Changed from 5
            'early_stopping_patience': 5,  # Changed from 8
            'use_amp': True,
            'num_workers': 0,  # Changed from 2
            'seed': 42,
            'warmup_epochs': 2,  # Changed from 3
...
def main():
    ...
    print(f"\n📊 Dataset sizes:")
    print(f"  Train: {len(train_data_file['flux'])} samples")
    print(f"  Val: {len(val_data_file['flux'])} samples")
    
    # Warn if dataset is large
    if len(train_data_file['flux']) > 200:
        print("\n⚠️  WARNING: Using large dataset configuration!")
        print("   This script is optimized for small datasets")
    ...
    optimizer = OptunaTrainerGPU(
        ...
        val_data=val_data,
        n_trials=30,  # Fewer trials to run faster
